...
type FileAdminToolbarProps = {
  search: string;
  onSearchChange: (value: string) => void;
  onCreate: () => void;
};

export default function FileAdminToolbar({
  search,
  onSearchChange,
  onCreate,
}: FileAdminToolbarProps) {
  return (
    <div className="flex flex-col gap-4 rounded-3xl border border-outline bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
      <div className="w-full max-w-xl">
        <label htmlFor="file-admin-search" className="sr-only">
          Rechercher un fichier admin
        </label>
        <div className="relative">
          <Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-secondary" size={18} />
          <input
            id="file-admin-search"
            value={search}
            onChange={(event) => onSearchChange(event.target.value)}
            placeholder="Rechercher par nom de la prestation"
